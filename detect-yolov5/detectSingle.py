...
def run(
    ...
    while True:
        ...
        source=('rtmp://127.0.0.1/{}p/test').format(str(cid))
        print(source)
        # source = str(cid)
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for (i, det), (i_2, det_2), (i_3, det_3), (i_4, det_4) in zip(enumerate(pred), enumerate(pred_2), enumerate(pred_3), enumerate(pred_4)):  # per image
                ...
                if len(det):
                    ...
                    if isPeo:# 多模型检测
                        ...
                        for *xyxy, conf, cls in reversed(det_4):
                            ...
                            info = str(x0) + ',' + str(y0)
                            if (float(f'{conf:.2f}') > 0.64 and cy!=True or float(f'{conf:.2f}') > 0.85):
                                det_list.append([object_name, xyxy, conf])  # 将标签位置存入数组
                            ...
                            if p[0] == 'person':
                                if isCalling(l[1], p[1]):
                                    ...
                                    if call_weigh < call_sub:
                                        call_date = datetime.datetime.now()
                                    if call_weigh < call_weigh_max:
                                        call_weigh = call_weigh + call_add * l[2] + call_sub
                                    break
                    if isCall:
                        break
                if call_weigh < call_min and (call_last or call_cnt > 0):
                    call_cnt += 1
                else:
                    call_cnt = 0
                ...
                if smoke_weigh >= smoke_sub and smoke_cnt <= smoke_max:
                    ...
                    f = open('record.txt', 'a')
                    print(datetime.datetime.now().strftime('%Y-%m-%d,%H:%M:%S') + '在' + smoke_date.strftime(
                            '%Y-%m-%d,%H:%M:%S') + '检测到' + str(smoke_num) + '次抽烟行为')
                    f.write(
                ...
                for l in det_list:
                    ...
                    if isSmoke:
                        break
                if smoke_weigh < smoke_min and (smoke_last or smoke_cnt > 0):
                    smoke_cnt += 1
                else:
                    smoke_cnt = 0
                ...
                if down_weigh >= down_sub and down_cnt <= down_max:
                    ...
                    f = open('record.txt', 'a')
                    print(datetime.datetime.now().strftime('%Y-%m-%d,%H:%M:%S') + '在' + down_date.strftime(
                            '%Y-%m-%d,%H:%M:%S') + '检测到' + str(down_num) + '次跌倒行为')
                    f.write(
                ...
                for l in det_list:
                    if l[0] == 'down':
                        ...
                        break
                if down_weigh < down_min and (down_last or down_cnt > 0):
                    down_cnt += 1
                else:
                    down_cnt = 0
                    ...
                    if dataset.mode == 'image':
                        ...
                        if seen%1800==0:
                            ...
                            thread1 = threading.Thread(target=function1, args=((df), ('runs/detect/test/' +tm+'-'+ str(lastseen) + '.csv'),
                                                                               (tm), str(1800), (tm+'-'+str(lastseen)+'.mp4'),
                                                                               (tm+'-'+str(lastseen) + '.csv'),))
                            thread1.start()
    ...
    thread1 = threading.Thread(target=function1,
                               args=((df), ('runs/detect/test/' + tm + '-' + str(lastseen) + '.csv'),
                                     (tm), str(seen%1800), (tm + '-' + str(lastseen) + '.mp4'),
                                     (tm + '-' + str(lastseen) + '.csv'),))
    thread1.start()
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='rtmp://127.0.0.1/1p/test', help='file/dir/URL/glob/screen/0(webcam)')
